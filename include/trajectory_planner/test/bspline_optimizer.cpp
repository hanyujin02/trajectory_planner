...
namespace ego_planner
{
  ...
  std::vector<std::vector<Eigen::Vector3d>> BsplineOptimizer::initControlPoints(Eigen::MatrixXd &init_points, bool flag_first_init /*= true*/)
  {
    ...
    for (size_t i = 0; i < segment_ids.size(); ++i)
    {
      ...
      {
        cout << "Astar " << i << " start: " << in.transpose() << " end: " << out.transpose() << endl;
                                                 ...
                                                 Eigen::MatrixXd &gradient, int iter_num, double smoothness_cost)
  {
    ...
    // 检查是否需要rebound，如果需要rebound，那么就会force stop
    ...
    for (auto i = order_; i < end_idx; ++i)
    {
      ...
      {
        double dist = (cps_.points.col(i) - cps_.base_point[i][j]).dot(cps_.direction[i][j]); // 负数的话说明在obstacle里，我们希望这个数值是正数并且大于等于我们设定的值
  ...
  bool BsplineOptimizer::check_collision_and_rebound(void)
  {
    ...
    // 重新找collision control points并且存在segment_ids中
    for (int i = order_ - 1; i <= i_end; ++i)
    {
      // 检查每一个control points是否有collision
      ...
      if (occ)
      {
        ...
        {
          cout.precision(2);
          // 这里计算的是control points对于obstacle的距离（注意负表示是在obstacle内【本意】，但实际上实在gradient的另一个halfspace，并不一定是在obstacle内）
      ...
      // 这一部分是找collision control points
      if (occ)
      {
    ...
    // 添加新的base points和gradient direction，这一部分和之前相同
  ...
  // 在initControlPoints后，在外部调用这个函数进行 优化
  // 注意：在这个函数中，cps的base_points和gradient已经都完成assignment
  ...
  bool BsplineOptimizer::rebound_optimize()
  {
    ...
    variable_num_ = 3 * (end_id - start_id); // 总共需要optimize的control points的数量，前三个和后三个是start和end
    double final_cost;
    ...
    do
    {
      ...
      // 调用lbfgs函数
      // 输入：
      // variable_num_: 输入的variable的总数
      // q: array表示的control points
      // final_cost: by reference, 目标函数的值
      // costFunction：重点，目标函数
      // earlyExit: 是否提早退出优化
      // lbfgs_params: 参数
          ...
          result == lbfgs::LBFGS_STOP)
      {
        ...
        double t_step = (tmp - tm) / ((traj.evaluateDeBoorT(tmp) - traj.evaluateDeBoorT(tm)).norm() / grid_map_->getResolution()); // 这个ts可以检查每个grid的occ info
  ...
  bool BsplineOptimizer::refine_optimize()
  {
    ...
    // 继续优化的条件：
    // 1. 轨迹的前三分之二有collision并且重新iteration的数目小于1
    do
    {
      lbfgs::lbfgs_parameter_t lbfgs_params;
      lbfgs::lbfgs_load_default_parameters(&lbfgs_params);
  ...
  {
    // 把x中的一维变量复制到cps里，在这一步中其实已经更新了cps
    ...
    // 总的cost，注意这里collision cost的系数是可变的
  ...
  {
    // 注意cps每次都会更新，就算优化没成功也会重新使用上次规划的轨迹，而不是原来的轨迹
    memcpy(cps_.points.data() + 3 * order_, x, n * sizeof(x[0]));
    ...
    // 这里只是把collision cost替换成了fitness cost
    calcSmoothnessCost(cps_.points, f_smoothness, g_smoothness);
