...
namespace ego_planner
{
  ...
  void EGOPlannerManager::initPlanModules(ros::NodeHandle &nh, PlanningVisualization::Ptr vis)
  {
    ...
    nh.param("manager/control_points_distance", pp_.ctrl_pt_dist, -1.0); // default is 0.4
  ...
  // 主要的规划函数
  // 输入：起始的pos，vel，acc，目标的pos，vel，没有acc。
  // flag_polyInit：是否使需要产生minsnap polynomial轨迹作为输入
                                        ...
                                        Eigen::Vector3d local_target_vel, bool flag_polyInit, bool flag_randomPolyTraj, Eigen::MatrixXd& output_cpts, double& output_ts, vector<Eigen::Vector3d>& input_point_set, Eigen::MatrixXd& controlPointBefore, std::vector<Eigen::Vector3d>& bsplineStartEndConditions)
  {
    ...
    /*** STEP 1: INIT ***/
    // 实际上ts只可能是前面一种表达式
    // 1.2 * control point预设距离/最大设定速度 = 1.2 * （0.4/2.0） = 0.24 
    ...
    // 当flag_regenerate = true 的时候会一直触发，当flag_regenerate == false表示成功
    do 
    {
      // 重置条件
      point_set.clear();
      ...
      // 如果是第一次尝试 或者 需要产生minsnap轨迹 或者 ？， 产生min snap poly轨迹
      if (flag_first_call || flag_polyInit || flag_force_polynomial /*|| ( start_pt - local_target_pt ).norm() < 1.0*/) // Initial path generated from a min-snap traj by order.
      {
        ...
        do
        {
          ...
          for (t = 0; t < time; t += ts)
          {
            ...
            if ((last_pt - pt).norm() > pp_.ctrl_pt_dist * 1.5) // 如果采样点之间的距离过大，则缩小dt时间
            {
              flag_too_far = true;
              break;
            }
      ...
      // 这种情况下不产生min snap轨迹而是直接重复利用之前的轨迹
      else // Initial path generated from previous trajectory.
      {

        double t;
        double t_cur = (ros::Time::now() - local_data_.start_time_).toSec(); // 获取之前轨迹的当前时间

        vector<double> pseudo_arc_length; // 用于记录之前轨迹长度
        ...
        // 估算剩余min snap轨迹的时间， 生成min snap轨迹
          ...
          while ((id <= pseudo_arc_length.size() - 2) && sample_length <= pseudo_arc_length.back())
          {
            if (sample_length >= pseudo_arc_length[id] && sample_length < pseudo_arc_length[id + 1])
            {
              point_set.push_back((sample_length - pseudo_arc_length[id]) / (pseudo_arc_length[id + 1] - pseudo_arc_length[id]) * segment_point[id + 1] +
                                  (pseudo_arc_length[id + 1] - sample_length) / (pseudo_arc_length[id + 1] - pseudo_arc_length[id]) * segment_point[id]); // interpolate 使得采样点均匀分布
        ...
        {
          flag_force_polynomial = true; // 如果用之前的轨迹失败，那就强行采用min snap重新生成
          flag_regenerate = true; // 重新生成轨迹
        }
      }
    ...
    pos.setPhysicalLimits(pp_.max_vel_, pp_.max_acc_, pp_.feasibility_tolerance_); // 给bspline轨迹传入速度和加速度的限制
    ...
    output_cpts = optimal_control_points;
    output_ts = ts;
    return true;
  }
  ...
  // 如果不符合feasibility的条件，那就重新refine trajectory
  // 输入：
  // traj：当前的轨迹
  // 起始条件
  // ratio：放慢倍率
  // ts: 时间步长
  // 输出：optimal_control_points
  bool EGOPlannerManager::refineTrajAlgo(UniformBspline &traj, vector<Eigen::Vector3d> &start_end_derivative, double ratio, double &ts, Eigen::MatrixXd &optimal_control_points)
  {
    ...
    // 还是用相同的时间间隔，但是把time step增大了ratio倍
    ...
    // 把时间增长轨迹的采样点重新拿来作为reference进行优化
