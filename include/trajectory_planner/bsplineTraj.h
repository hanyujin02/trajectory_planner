...
namespace trajPlanner{
	...
	public:
		...
		bool inputPathCheck(const nav_msgs::Path & path, nav_msgs::Path& adjustedPath, double dt, double& finalTime, const bool &adjustLength = true);
		bool fillPath(const nav_msgs::Path& path, nav_msgs::Path& adjustedPath);
		bool updatePath(const nav_msgs::Path& adjustedPath, const std::vector<Eigen::Vector3d>& startEndConditions, const bool &adjustLength = true);
